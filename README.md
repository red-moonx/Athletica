...
- **Result:** The agent now reasons about its initial search results. If the retrieved chunks are irrelevant, it automatically triggers a second or third search with different keywords before answering. This drastically improved perceived relevance and user experience. I also added two custom mathematical tools (`calculate_energy_availability` and `issn_dosage_safety_checker`) to extend the agent's capabilities beyond mere text retrieval.
